...
#define ADA4255_CRC_POLYNOMIAL			0x7
DECLARE_CRC8_TABLE(ada4255_crc8_table);
...
struct ada4255_state {
	...
	u8				reg_buf[3] __aligned(IIO_DMA_MINALIGN);
};
...
static int ada4255_crc(u8 *buf)
{
	return crc8(ada4255_crc8_table, buf, 2, 0);
}

static int ada4255_reg_write(void *context, unsigned int reg, unsigned int val)
{
	struct ada4255_state *st = context;

	st->reg_buf[0] = reg;
	st->reg_buf[1] = val;
	st->reg_buf[2] = ada4255_crc(st->reg_buf);

	return spi_write(st->spi, st->reg_buf, sizeof(st->reg_buf));
}

static int ada4255_crc_check(struct ada4255_state *st, u8 *buf)
{
	struct device *dev = &st->spi->dev;
	u8 expected_crc = ada4255_crc(buf);

	if (buf[2] != expected_crc) {
		dev_err(dev, "Bad CRC %02x for %02x%02x, expected %02x\n",
			buf[2], buf[0], buf[1], expected_crc);
		return -EINVAL;
	}
...
static int ada4255_reg_read(void *context, unsigned int reg, unsigned int *val)
{
	struct ada4255_state *st = context;
	int ret;

	st->reg_buf[0] = reg | ADA4255_READ_FLAG_MASK;

	ret = spi_write_then_read(st->spi, st->reg_buf, 1, st->reg_buf, 2);
	if (ret)
		return ret;

	ret = ada4255_crc_check(st, st->reg_buf);
	if (ret)
		return ret;
...
static const struct regmap_config ada4255_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
	.reg_read = ada4255_reg_read,
	.reg_write = ada4255_reg_write,
};
...
static int ada4255_probe(struct spi_device *spi)
{
	...
	st->regmap = devm_regmap_init(dev, NULL, st, &ada4255_regmap_config);
	if (IS_ERR(st->regmap))
